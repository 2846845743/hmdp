...
//    }
    ...
    //处理落盘任务的具体逻辑，传递一个优惠券订单，数据库去刷盘.改造成mq
    ...
    public Result seckillVoucher(Long voucherId) {
        ...
        //拥有购买资格了，并且redis的库存也扣减了
        //todo 往消息队列里面添加消息，里面要存：订单id，秒杀券id，用户id。
        ...
        ObjectMapper objectMapper = new ObjectMapper();
        try {
            String jsonString = objectMapper.writeValueAsString(voucherOrder);
            log.info("消息的json字符串:{}",jsonString);
            String queue= "simple.queue";
            rabbitTemplate.convertAndSend(queue,null, jsonString);
        } catch (Exception e) {
            e.printStackTrace();
        }
